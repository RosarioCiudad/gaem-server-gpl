...
var ReporteTotalActas = function () {
	var module = {};
	...
	var SqlTmpResultado  = 
		...
		"SELECT usu.nombre inspector, SUM(CASE WHEN form.idEstadoTipoFormulario = 3 THEN 1 ELSE 0 END) actascorrectas," +
		"SUM(CASE WHEN form.idEstadoTipoFormulario = 4 THEN 1 ELSE 0 END)  actasanuladas," +
		" to_char(sum(form.fechacierre - form.fechainicio)/count(*), 'HH24 hrs MI \"min\" SS sec') promedio " +
...
//			" AND form.estado = 2" +
	...
	var SqlTmpPromedio = 
		...
		"SELECT " +
		" to_char(sum(form.fechacierre - form.fechainicio)/count(*), 'HH24 hrs MI \"min\" SS sec') prom, " +
		" to_char(max(form.fechacierre - form.fechainicio), 'HH24 hrs MI \"min\" SS sec') max, " +
		" to_char(min(form.fechacierre - form.fechainicio), 'HH24 hrs MI \"min\" SS sec') min " +
...
//			out.include("<grs>/html/exthead.html", dataMap);
		...
		out.thead(["Inspector", "Actas Correctas","Actas Anuladas", "Tiempo Promedio de Labrado "]);
		...
		while (row = cursor.read()) {
		        ...
		        if (row.inspector) row.inspector = row.inspector.toString().replace(",", " ", "g");
