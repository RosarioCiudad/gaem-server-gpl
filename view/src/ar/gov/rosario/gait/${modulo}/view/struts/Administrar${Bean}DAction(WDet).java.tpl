package ar.gov.rosario.gait.${modulo}.view.struts;
...
import ar.gov.rosario.gait.${modulo}.iface.model.${Bean}Adapter;
import ar.gov.rosario.gait.${modulo}.iface.model.${Bean}VO;
import ar.gov.rosario.gait.${modulo}.iface.service.${Modulo}ServiceLocator;
import ar.gov.rosario.gait.${modulo}.iface.util.${Modulo}Error;
import ar.gov.rosario.gait.${modulo}.iface.util.${Modulo}SecurityConstants;
import ar.gov.rosario.gait.${modulo}.view.util.${Modulo}Constants;
...
public final class Administrar${Bean}DAction extends BaseDispatchAction {
	...
	public ActionForward inicializar(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (navModel.getAct().equals(BaseConstants.ACT_VER)) {
				...
				${bean}AdapterVO = ${Modulo}ServiceLocator.get${Submodulo}Service().get${Bean}AdapterForView
			...
			if (navModel.getAct().equals(BaseConstants.ACT_MODIFICAR)) {
				...
				${bean}AdapterVO = ${Modulo}ServiceLocator.get${Submodulo}Service().get${Bean}AdapterForUpdate
					(userSession, commonKey);
				actionForward = mapping.findForward(${Modulo}Constants.FWD_${BEAN}_ADAPTER);
			}
			if (navModel.getAct().equals(BaseConstants.ACT_ELIMINAR)) {
				stringServicio = "get${Bean}AdapterForDelete(userSession, commonKey)";
				${bean}AdapterVO = ${Modulo}ServiceLocator.get${Submodulo}Service().get${Bean}AdapterForView
					(userSession, commonKey);
			...
			if (${bean}AdapterVO.hasErrorNonRecoverable()) {
				log.error("error en: "  + funcName + ": servicio: " + 
					stringServicio + ": " + ${bean}AdapterVO.errorString()); 
			...
			if (log.isDebugEnabled()) log.debug(funcName + ": " + 
				${Bean}Adapter.NAME + ": " + ${bean}AdapterVO.infoString());
	...
	public ActionForward modificarEncabezado(ActionMapping mapping, ActionForm form,
		...
		return baseForwardAdapter(mapping, request, funcName, 
			${Modulo}Constants.ACTION_ADMINISTRAR_ENC_${BEAN}, BaseConstants.ACT_MODIFICAR);
	...
	public ActionForward eliminar(ActionMapping mapping, ActionForm form,
		...
		String funcName = DemodaUtil.currentMethodName();
		if (log.isDebugEnabled()) log.debug("entrando en " + funcName);
		UserSession userSession = canAccess(request, mapping, ${Modulo}SecurityConstants.ABM_${BEAN}, 
		...
		try {
			...
			if (${bean}AdapterVO == null) {
				log.error("error en: "  + funcName + ": " + ${Bean}Adapter.NAME + 
	...
	public ActionForward refill(ActionMapping mapping, ActionForm form,
		...
		String funcName = DemodaUtil.currentMethodName();
		return baseRefill(mapping, form, request, response, funcName, ${Bean}Adapter.NAME);
		
	}
	
	// Metodos relacionados ${Bean_Detalle}
	public ActionForward ver${Bean_Detalle}(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response)
		throws Exception {

		String funcName = DemodaUtil.currentMethodName();
		return forwardVerAdapter(mapping, request, funcName, ${Modulo}Constants.ACTION_ADMINISTRAR_${BEAN_DETALLE});

	}

	public ActionForward modificar${Bean_Detalle}(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response)
		throws Exception {

		String funcName = DemodaUtil.currentMethodName();
		return forwardModificarAdapter(mapping, request, funcName, ${Modulo}Constants.ACTION_ADMINISTRAR_${BEAN_DETALLE});

	}

	public ActionForward eliminar${Bean_Detalle}(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response)
		throws Exception {

		String funcName = DemodaUtil.currentMethodName();
		return forwardEliminarAdapter(mapping, request, funcName, ${Modulo}Constants.ACTION_ADMINISTRAR_${BEAN_DETALLE});

	}
	
	public ActionForward agregar${Bean_Detalle}(ActionMapping mapping, ActionForm form,
		HttpServletRequest request, HttpServletResponse response)
		throws Exception {

		String funcName = DemodaUtil.currentMethodName();
		return forwardAgregarAdapter(mapping, request, funcName, ${Modulo}Constants.ACTION_ADMINISTRAR_${BEAN_DETALLE});
