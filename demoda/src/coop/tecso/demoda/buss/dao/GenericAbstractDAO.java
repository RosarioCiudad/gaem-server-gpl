...
public abstract class GenericAbstractDAO{
    ...
    /* Si es Versionable, incrementamos el valor de version en 1 */
    public synchronized Long update(VersionableBO o) throws HibernateException {
    	return update((BaseBean) o);
    }
	...
	public List executeCountedSearch(QueryMaker queryMaker, PageModel page) throws Exception {
		return executeCountedSearch(queryMaker.queryString(), queryMaker.parameters(), page);
		
	}

	public List executeCountedSearch(String queryString, PageModel page) throws Exception{
		return executeCountedSearch(queryString, null, page);
	}

	public List<Common> executeCountedSearch(String queryString, List<Object> parameters, PageModel page) throws Exception{
		if (page.getReport().getImprimir()){
	    ...
	    query = HibernateQueryMaker.query(session, "SELECT COUNT(*) " + queryCount, parameters);

	    cantidadMaxima = (Long) query.uniqueResult();
	    page.setMaxRegistros(cantidadMaxima);

		// obtenemos el resultado de la consulta
	    query = HibernateQueryMaker.query(session, queryString, parameters);
	...
	*/	
	...
	private void executePrintSearch(String queryString, PageModel page) throws Exception {
		executePrintSearch(queryString, null, page);
	}
	 ...
	 * @throws Exception
	 */
	private void executePrintSearch(String queryString, List<Object> parameters, PageModel page) throws Exception {
		...
		if (posFrom > 0){
			...
			if(posSelect >= 0){
				...
				query = HibernateQueryMaker.query(session, "SELECT COUNT( " + ejecucion + " ) "+ posteriorFrom, parameters);
			}
		}else{
			query = HibernateQueryMaker.query(session, "SELECT COUNT(*) " + queryCount, parameters);
		}
		...
		List listBO = HibernateQueryMaker.query(session, queryString, parameters).list();
		
		ContenedorVO contenedorPrincipal = cargarContenedor(listBO, page.getReport());
		imprimirGenerico(contenedorPrincipal, page.getReport());
	}
	...
	*/
	public boolean hasReferenceActivoGen(BaseBean bo, Class joinClass, String joinProperty) {
		String queryString = "from %s t where t.%s = :bo and t.estado = 1";
		Session session = currentSession();
		Query query = session.createQuery(String.format(queryString, joinClass.getName(), joinProperty));
		query.setEntity("bo", bo);
    	List list = (List) query.list();
	...
	*/
	public boolean checkIsUniqueGen(BaseBean obj, UniqueMap um) throws Exception {
		...
		while (it.hasNext()) {
			...
			if (value.toString().equals(UniqueMap.REFLECT_COOKIE)) {
	...
	*/
	...
	public PlanillaVO imprimirGenerico(ContenedorVO contenedorPrincipal, ReportVO report) throws Exception{
		...
		String idUsuario = StringUtil.formatLong(DemodaUtil.currentUserContext().getIdUsuarioGait());
	...
	public boolean checkConcurrencyGen(BaseBean obj, UniqueMap claveFuncional, String userName) throws Exception {
		...
		// TODO: queryString += " and t.fechaUltMdf = " + sqlDate(new Date()) + ")";
