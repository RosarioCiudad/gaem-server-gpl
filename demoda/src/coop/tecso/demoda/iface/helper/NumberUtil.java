...
public class NumberUtil {
	  ...
	  /**
	   * Recibe un Double y Devuelve un String que lo representa en palabras.<br>
	   * Utiliza como separador de comas el "." y como moneda "pesos".<br>
	   * @param numero
	   * @param toUpperCase si es "true" devuelve la cadena en MAYUSCULAS
	   * @return
	   * @author leonardo.fagnano
	   */
	  public static String getNroEnPalabras(Double numero, int decimales, boolean toUpperCase){
		  return getNroEnPalabras(numero, decimales, "pesos", toUpperCase);
	  }
	  ...
	  /**
	   *  GAITMRM-71 : Modificaciones a Proceso Masivo para disponibilizar a Procuradores
	   *  Se crea este metodo para poder formatear doubles que están en notación cientifica, es decir
	   *  numeros muy chicos o muy grandes.
	   * 
	   * @param numero
	   * @param decimales
	   * @param moneda
	   ...
	   * @return
	   */
	  public static String getNroEnPalabras(Double numero, int decimales, String moneda, boolean toUpperCase){
		  
		  if(numero==null || numero.doubleValue()<=0)
			  return "";
		  
		  String formatoDouble = "0." + StringUtil.devuelveCadena("0", decimales); 
		  
		  String strNro = StringUtil.formatDouble(numero, formatoDouble);
		  int posicionComa = strNro.indexOf(".");
		  if(posicionComa<0)
			  return "";
		  String parteEntera = strNro.substring(0, posicionComa);
		  String parteDecimal = strNro.substring(posicionComa+1);
		  ...
		  String nroEnPalabras=getNroEnPalabras2(new BigDecimal(parteEntera))+" "+moneda+" con "+getNroEnPalabras2(new BigDecimal(parteDecimal))+"  centavos";
		  return (toUpperCase?nroEnPalabras.toUpperCase():nroEnPalabras);
	  }
		 ...
		 */
		public static String getNroEnPalabras2(BigDecimal numero) {
		    String palabras = "";
		...
		//	    r9=r8%1;
		        ...
		        if (n<1000000000 && n>=1000000) {
		            palabras = letras(cm,dm,um) + " Millones ";
		            if (!StringUtil.isNullOrEmpty(letras(cmi,dmi,umi)))
		            	palabras+= letras(cmi,dmi,umi) + " Mil ";
		            if (!StringUtil.isNullOrEmpty(letras(ce,de,un)))
		            	palabras+= letras(ce,de,un);
		            else if (StringUtil.isNullOrEmpty(letras(cmi,dmi,umi)) && StringUtil.isNullOrEmpty(letras(ce,de,un)))
		            	palabras+= " de ";
		        }
