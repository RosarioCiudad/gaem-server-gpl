...
public class DateUtil {
	...
	public static String ddbMMbYYYY_HH_MM_SS_MASK = "dd MM yyyy HH:mm:ss";  
    ...
    // Devuelve la fecha correspondiente al Domingo de la semana siguiente de "fecha"
    public static Date getFirstDayOfNextWeek(Date fecha) {
    	Calendar fechaC = new GregorianCalendar();
    	fechaC.setTime(fecha);
    	if (fechaC.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY)
    		fechaC.add(Calendar.DAY_OF_MONTH, 1);
    	while (fechaC.get(Calendar.DAY_OF_WEEK) != Calendar.SUNDAY) 
    		fechaC.add(Calendar.DAY_OF_MONTH, 1 );	
    ...
    public static Date getCurrentWeekFriday(Date fecha) {
    	Calendar fechaC = new GregorianCalendar();
    	fechaC.setTime(fecha);
    	
    	while (fechaC.get(Calendar.DAY_OF_WEEK) != Calendar.FRIDAY) {
    		fechaC.add(Calendar.DAY_OF_MONTH, 1 );	
		}
    	return getDateFromDate (fechaC.getTime());
    }
    ...
    public static double getCantMeses(Date fecha1, Date fecha2) {
    	...
    	while(!salir){
    		if(gc1.before(gc2)){
    			...
    			elapsed +=cantDias/30.44D; // segun google: 1 mes = 30,4368499 días
    			salir=true;
    		}
    	}
	...
	public static boolean isValidAnio(Integer anio ) {
	 ...
	 /**
     * Obtiene una lista de pares de Fechas que marcan el principio y fin de los rangos de meses por año calculados entre las fecha desde y hasta indicadas.
     * 
     * @param fechaDesde
     * @param fechaHasta
     * @return List<Date[]>
	...
	public static XMLGregorianCalendar getXMLGCDate(Date dateTo) throws Exception {
		if (dateTo == null)
			return null;
		try {
			GregorianCalendar gregCal = new GregorianCalendar();
			gregCal.setTime(dateTo);
